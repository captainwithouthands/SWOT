export type SectionKey = "english" | "legal" | "logical" | "currentAffairs" | "quant";
...
export const QUESTION_TYPES: Record<
  SectionKey,
  { key: string; label: string; drillAdvice: string }[]
> = {
  english: [
    {
      key: "inference",
      ...
      drillAdvice:
        "Stay within the passage. Wrong inferences usually import outside knowledge — only use what's explicitly written.",
    },
    {
      key: "main_idea",
      label: "Main idea / Central theme",
      drillAdvice:
        "Read first and last sentences of each paragraph first. The central theme is almost always explicitly stated.",
    },
    {
      key: "vocab_context",
      label: "Vocabulary in context",
      drillAdvice:
        "Use elimination. Substitute each option and ask 'does this preserve the author's meaning?' Not dictionary definitions.",
    },
    {
      key: "tone_attitude",
      label: "Author's tone / Attitude",
      drillAdvice:
        "Look for emotionally loaded words (adjectives, adverbs). The tone must match ALL charged words in the passage, not just one.",
    },
    {
      key: "fill_blank",
      label: "Fill in the blank",
      drillAdvice:
        "Check grammatical clues (connectors like 'however/therefore', singular/plural agreement) before reading the options.",
    },
    {
      key: "grammar",
      label: "Grammar / Usage",
      drillAdvice:
        "Focus on subject-verb agreement, pronoun reference, and misplaced modifiers — these three cover 80% of CLAT grammar questions.",
    },
    {
      key: "title_heading",
      label: "Title / Heading",
      drillAdvice:
        "The title must capture the ENTIRE passage, not just one paragraph. Eliminate options that are too narrow or too broad.",
    },
    {
      key: "summary",
      label: "Summary / Para completion",
      drillAdvice:
        "A valid summary must include the main claim and key support. If it contradicts any part of the passage, eliminate it.",
    },
  ],
  currentAffairs: [
    {
      key: "static_gk",
      label: "Static GK (Constitution / Polity)",
      drillAdvice:
        "Revise constitutional articles weekly. Articles 12–35 (FRs), 36–51 (DPSPs), and all Schedules are the most tested areas.",
    },
    {
      key: "current_news",
      label: "Current Affairs (News-based)",
      drillAdvice:
        "Read Hindu/IE editorials for the 6 months before the exam. Focus on policy decisions and court rulings, not event dates.",
    },
    {
      key: "legal_gk",
      label: "Legal GK (Acts / Landmark Cases)",
      drillAdvice:
        "Know year + key holding of: Kesavananda Bharati, Maneka Gandhi, Vishaka, Indra Sawhney, and the 5 latest landmark cases.",
    },
    {
      key: "science_tech",
      label: "Science & Technology",
      drillAdvice:
        "Follow PIB for space (ISRO), defence, and health-tech announcements. CLAT tests applications, not deep science.",
    },
    {
      key: "intl_affairs",
      label: "International Affairs",
      drillAdvice:
        "Track UN resolutions, G20/SCO outcomes, and bilateral agreements India signed in the past year.",
    },
    {
      key: "govt_schemes",
      label: "Govt Schemes / Policies",
      drillAdvice:
        "Revise PM-launched flagship schemes with their objectives and target years — not just the names.",
    },
    {
      key: "sports_awards",
      label: "Sports / Awards / Appointments",
      drillAdvice:
        "Use a monthly current affairs PDF. This sub-type rewards consistent revision — it cannot be crammed last-minute.",
    },
  ],
  legal: [
    {
      key: "principle_fact",
      label: "Principle–Fact application",
      drillAdvice:
        "Read the principle twice before the facts. Identify: what legal rule is triggered? Then apply it mechanically — no moral reasoning.",
    },
    {
      key: "principle_id",
      label: "Principle identification",
      drillAdvice:
        "Ask 'what is the law protecting here?' The answer is usually one of: property, consent, bodily autonomy, or reasonable reliance.",
    },
    {
      key: "legal_maxim",
      label: "Legal maxims / Latin terms",
      drillAdvice:
        "Memorise top 20 CLAT maxims with one real case example each. Recognition beats rote translation.",
    },
    {
      key: "contract",
      label: "Contract law",
      drillAdvice:
        "Core: offer-acceptance-consideration, void vs voidable, mistake, misrepresentation. Apply ICA 1872 mentally while reading.",
    },
    {
      key: "tort",
      label: "Tort law",
      drillAdvice:
        "Revise: negligence (duty-breach-damage), nuisance, strict liability (Rylands v Fletcher), defamation. CLAT rarely goes beyond these five.",
    },
    {
      key: "criminal",
      label: "Criminal law (IPC / BNS)",
      drillAdvice:
        "Focus on mens rea vs actus reus, ss. 299–300 (murder/culpable homicide), ss. 378–391 (theft), and general exceptions ss. 76–106.",
    },
    {
      key: "constitutional",
      label: "Constitutional law",
      drillAdvice:
        "CLAT tests application of FRs + DPSPs. For every Q: identify the right, who's violating it, then apply the reasonable-restriction test.",
    },
    {
      key: "family_property",
      label: "Family / Property law",
      drillAdvice:
        "Know Hindu Succession Act basics, Muslim personal law essentials, and Transfer of Property Act key sections — frequently in mixed passages.",
    },
    {
      key: "intl_law",
      label: "International law",
      drillAdvice:
        "Focus on: UN Charter, sovereignty principles, pacta sunt servanda, and recent ICJ cases involving India.",
    },
  ],
  logical: [
    {
      key: "strengthen_weaken",
      label: "Strengthen / Weaken argument",
      drillAdvice:
        "Map the conclusion before reading options. A strengthener adds evidence FOR the conclusion; a weakener attacks the underlying assumption.",
    },
    {
      key: "assumption",
      label: "Assumption identification",
      drillAdvice:
        "Use the negation test: negate the option. If the argument collapses, that's the assumption.",
    },
    {
      key: "inference_draw",
      label: "Inference drawing",
      drillAdvice:
        "A valid inference must be 100% supported by the passage. If it's only 'likely', it fails the test.",
    },
    {
      key: "cause_effect",
      label: "Cause-effect analysis",
      drillAdvice:
        "True causation requires: A precedes B, A is necessary for B, and no confounding factor explains B. Check all three.",
    },
    {
      key: "flaw_identify",
      label: "Flaw / Error identification",
      drillAdvice:
        "Name the flaw type first: ad hominem, hasty generalisation, false analogy, circular reasoning. Once named, match it to options.",
    },
    {
      key: "analogy",
      label: "Analogy / Parallel reasoning",
      drillAdvice:
        "Find the logical structure (A:B as C:D). Match the relationship, not the domain or content.",
    },
    {
      key: "sequence",
      label: "Logical sequence / Arrangement",
      drillAdvice:
        "Find the anchor — the statement that can only go first or last. Build outward from there, not from the middle.",
    },
    {
      key: "syllogism",
      label: "Syllogism / Statement-conclusion",
      drillAdvice:
        "Use Venn diagrams for All-Some-No questions. Never assume what's not explicitly stated.",
    },
  ],
  quant: [
    {
      key: "data_interp",
      label: "Data Interpretation (tables / charts)",
      drillAdvice:
        "Approximate first (round to 2 significant figures). CLAT DI options are spaced enough that approximation finds the right answer.",
    },
    {
      key: "percentage",
      label: "Percentage / Ratio / Proportion",
      drillAdvice:
        "Convert to fractions immediately. 37.5% = 3/8. Mental fraction arithmetic is 3× faster than percentage-based calculation.",
    },
    {
      key: "time_speed",
      label: "Time–Speed–Distance / Work",
      drillAdvice:
        "Use the unitary method. Establish base rate first, then scale. Relative speed = sum (opposite) or difference (same direction).",
    },
    {
      key: "profit_loss",
      label: "Profit–Loss / SI & CI",
      drillAdvice:
        "For P&L, set cost price = 100. For successive discounts, multiply fractions — never add percentages.",
    },
    {
      key: "number_series",
      label: "Number series / Patterns",
      drillAdvice:
        "Check differences, ratios, squares, and cubes in that order. Most CLAT series follow exactly one of these four patterns.",
    },
    {
      key: "geometry",
      label: "Geometry / Mensuration",
      drillAdvice:
        "Memorise area and volume formulas cold. CLAT tests: circles, triangles (Heron's formula), and cylinders most often.",
    },
    {
      key: "averages",
      label: "Averages / Mixtures / Alligation",
      drillAdvice:
        "For alligation: draw the cross-multiply diagram. For weighted averages: sum of (weight × value) ÷ total weight.",
    },
    {
      key: "sets_logic",
      label: "Sets / Venn diagrams / Probability",
      drillAdvice:
        "Always draw the Venn diagram — never solve mentally. Label all regions including the 'outside' region.",
    },
  ],
};
...
export function buildSwot(
  ...
  sections.forEach((s) => {
    ...
    withWrongs
      ...
      .slice(0, 2)
      .forEach((t) => {
        weaknesses.push(`${s.name} → ${t.label} (${t.wrongs} wrong): ${t.drillAdvice}`);
      });
    ...
    if (nearMiss.length >= 2) {
      opportunities.push(
        `${s.name}: near-misses on ${nearMiss
          .slice(0, 3)
          .map((t) => t.label)
          .join(
            ", ",
          )} (1 wrong each) — a targeted 30-min drill per type would likely clean these up.`,
      );
    ...
    if (hardZero.length >= 2 && s.attempted > 0) {
      strengths.push(
        `${s.name}: zero errors on ${hardZero
          .slice(0, 2)
          .map((t) => t.label)
          .join(
            " & ",
          )} — these are the hardest question types in this section. This is a genuine competitive edge.`,
      );
    }
  });
  ...
  if (!strengths.length)
    strengths.push(
      "No section crossed the 75% accuracy benchmark yet — your first priority is building one reliable anchor section.",
    );
  if (!weaknesses.length)
    weaknesses.push(
      "No critical weakness — focus on widening the gap from cutoff rather than damage control.",
    );
  if (!opportunities.length)
    opportunities.push(
      "Attempt rates are healthy — sharpen elimination and timing instead of raw volume.",
    );
  if (!threats.length)
    threats.push(
      "Negative marking under control. Lock in this attempt discipline across the next 3 mocks.",
    );
...
export const COHORT_PRESETS: {
  ...
  { label: "CLAT 2026 (75k)", size: 75000, hint: "Actual exam size", kind: "national" },
];
...
export function computeRankAndPercentile(score: number, opts: PercentileOptions): RankResult {
  ...
  const rank = Math.min(
    cohortSize,
    Math.max(1, Math.round((1 - cappedP / 100) * cohortSize + 0.5)),
  );
  const percentile = +((1 - (rank - 0.5) / cohortSize) * 100).toFixed(2);
...
export function scoreForPercentile(
  ...
  _cohortSize: number = BASELINE_COHORT,
): number {
